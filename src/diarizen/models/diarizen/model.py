...
from diarizen.models.diarizen.conformer import ConformerEncoder
from diarizen.models.diarizen.receptive_field import (
    multi_conv_num_frames,
    multi_conv_receptive_field_center,
    multi_conv_receptive_field_size,
)
from diarizen.models.diarizen.wav2vec2.model import wavlm_model
from diarizen.models.diarizen.wavlm_config import get_config
...
class Model(nn.Module):
    def __init__(
        ...
        self.classifier = nn.Linear(
            attention_in, self.dimension(max_speakers_per_chunk, 2)
        )
    ...
    @staticmethod
    def dimension(classes, powerset_max_classes) -> int:
        """Dimension of output"""
        return int(
            sum(
                scipy.special.binom(classes, i)
                for i in range(0, powerset_max_classes + 1)
            )
        )
