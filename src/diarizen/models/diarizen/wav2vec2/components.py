...
class WavLMSelfAttention(SelfAttention):
    ...
    def forward(
        ...
        if attention_mask is not None:
            attn_mask = self._merge_masks(attn_mask_rel_pos, attention_mask)
    ...
    @torch.jit.export
    def _merge_masks(
        self, rel: Optional[torch.Tensor], vis: Optional[torch.Tensor]
    ) -> torch.Tensor:
        if rel is None:
            assert vis is not None, "At least one of the masks must be provided."
            return vis
        elif vis is None:
            assert rel is not None, "At least one of the masks must be provided."
            return rel
        else:
            return rel + vis
